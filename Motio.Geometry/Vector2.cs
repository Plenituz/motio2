...
namespace Motio.Geometry
{
    ...
    public struct Vector2 : IEquatable<Vector2>
    {
        ...
        {
            value.X = (float)(value.X * scaleFactor);
            value.Y = (float)(value.Y * scaleFactor);
            return value;
        }
        ...
        {
            //(a,b) => (-b,a) rotate 90° if the point is 2D
            float tmp = X;
            X = -Y;
            Y = tmp;
        }

        public static Vector2? LinesIntersection(Vector2 p1, Vector2 d1, Vector2 p2, Vector2 d2)
        {
            //we reuse p1
            if (!LinesIntersection(ref p1, ref d1, ref p2, ref d2, out p1))
                return null;
            else
                return p1;
        }
        ...
        /// <summary>
        /// intersections between 2 lines. Lines are defined by a point and a vector 
        /// </summary>
        /// <param name="p1">point on the first line</param>
        /// <param name="d1">direction vector of the first line</param>
        /// <param name="p2">point on the second line</param>
        /// <param name="d2">direction vector of the second line</param>
        /// <param name="result">vector2 to store the result in if there is one</param>
        /// <returns>false if the lines were parallel and an intersection couldn't be found, true otherwise</returns>
        public static bool LinesIntersection(ref Vector2 p1, ref Vector2 d1, ref Vector2 p2, ref Vector2 d2, out Vector2 result)
        {
            if (d1 == d2 || d1 == -d2)
            {
                result = Zero;
            ...
            float t = ( (p2.Y - p1.Y)*d1.X + (p1.X - p2.X)*d1.Y ) / (d2.X*d1.Y - d2.Y*d1.X);
            result = p2 + d2 * t;
            return Sanitize(result);
        }

        public static bool Sanitize(Vector2 vector)
        {
